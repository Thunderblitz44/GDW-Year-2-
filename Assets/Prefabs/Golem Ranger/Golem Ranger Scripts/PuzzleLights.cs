...
public class PuzzleLights : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < lineRenderers.Length; i++)
        {
            ...
            lineRenderers[i].gameObject.SetActive(false); // Initially deactivate the line renderers
        }

        // Start the coroutine to activate line renderers with a delay
        StartCoroutine(ActivateLineRenderersWithDelay());
    }

    IEnumerator ActivateLineRenderersWithDelay()
    {
        for (int i = 0; i < lineRenderers.Length; i++)
        {
            yield return new WaitForSeconds(1f); // Wait for 1 second before activating the next line renderer
